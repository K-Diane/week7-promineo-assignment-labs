...
const food = "chicken biryani rice";
let drink = "cocacola";

//step 2 using 'let'//

let myName = "John";
let myFriendName = "Jake";

console.log(food, drink, myName, myFriendName);

//Question: Name a reason why you would use const instead of let.
//Answer: i will use const in case i want the value not to change
...
console.log(
  `${myName} and ${myFriendName} will have on lunch ${food} and will drink ${drink}`
);
...
//Answer: the new varibla is not defined as we tried to reassigned varibla with "const"
...
let lunchTime = (foodChoice, drinkChoice) => {
  return `${myName} and ${myFriendName} will have on lunch ${foodChoice} and will drink ${drinkChoice}`;
};
console.log(lunchTime("Pizza", "Water"));
...
console.log(
  foodForTodayAndTomorrow(
    "pizza",
    "soda",
    lunchTime("newFoodItem", "newDrinkItem")
  )
);
